...
/* ---------- queries ---------- */
const execBody = z.object({ sql: z.string().min(1).max(MAX_SQL_LENGTH), useExplain: z.boolean().optional(), targetNode: z.string().optional() });
dashboardRouter.post("/query/execute", async (req, res, next) => {
  ...
  try {
    ...
    const { result } = await executeQuery(sql, [], { forcePrimary: b.targetNode === "primary" });
