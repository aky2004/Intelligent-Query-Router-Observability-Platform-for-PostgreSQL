...
async function deliver(job:EmailJob){const apiKey=process.env.LOVABLE_API_KEY,connectionKey=process.env.BREVO_API_KEY;if(!apiKey||!connectionKey){logger.warn("Brevo connection unavailable; email not delivered",{to:job.to});return;}const response=await fetch("https://connector-gateway.lovable.dev/brevo/v3/smtp/email",{method:"POST",headers:{"content-type":"application/json",Authorization:`Bearer ${apiKey}`,"X-Connection-Api-Key":connectionKey},body:JSON.stringify({sender:{name:process.env.FROM_NAME??"Pg-admin",email:process.env.FROM_EMAIL??"noreply@example.com"},to:[{email:job.to}],subject:job.subject,htmlContent:job.html})});if(!response.ok)throw new Error(`Brevo delivery failed [${response.status}]: ${await response.text()}`);}
